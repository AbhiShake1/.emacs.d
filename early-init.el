...
(defun flutter-run-or-hot-reload-gharlens (&optional args)
  "Start `flutter run` or hot-reload if already running."
  (interactive)
  (if (flutter--running-p)
      (flutter-hot-reload)
    (flutter-run '"--dart-define=flavor=dev --dart-define=dev_id=AP --flavor dev")))

(use-package ob-dart
:straight (:host github :repo "mzimmerm/ob-dart"))

(use-package dart-mode
:straight (:host github :repo "emacsorphanage/dart-mode")
:config
(add-hook 'dart-mode-hook
          (lambda ()
            (add-hook 'after-save-hook 'flutter-run-or-hot-reload-gharlens))))

(use-package lsp-dart
:straight (:host github :repo "emacs-lsp/lsp-dart")
:config
...
:config
(add-hook 'rust-mode-hook (lambda () (pretty-symbols-mode)))
...
(use-package flutter
  :straight (:host github :repo "amake/flutter.el")
  :after dart-mode
  :config
  :custom
    (add-to-list 'auto-mode-alist '("\\.dart" . dart-mode))
  (flutter-sdk-path "/sbin/flutter"))
